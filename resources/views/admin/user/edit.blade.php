		...
		<div class=" layui-row layui-col-space10">
				...
				<div class="border magt3">
					<div class="layui-form-item ">
						...
						<div class="layui-input-block">
							<input type="text" class="layui-input" lay-verify="content" name="nickname" placeholder="请输入用户昵称"  value="{{$detail['nickname']}}">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">用户名</label>
						<div class="layui-input-block">
							<input type="text" class="layui-input" lay-verify="required" name="username" placeholder="用户名"  value="{{$detail['username']}}">
						</div>
					</div>
					<div class="layui-form-item">
						<label class="layui-form-label">用户密码</label>
						<div class="layui-input-block">
							<input type="text" class="layui-input"  name="password" placeholder="塡值即修改">
						</div>
					</div>
				</div>
			</div>
			<div class="layui-col-md3 layui-col-xs5">
				...
				<div class="layui-upload-list thumbBox mag0 magt3"  id="thumbImg">
					<img class="layui-upload-img thumbImg" src="{{$detail['avatar'] ? '/'.$detail['avatar'] : ''}}">
				</div>
				<input type="hidden" name="avatar" lay-verify="imgpath" id="filepath" value="{{$detail['avatar']}}">
			</div>
	...
	<div class="layui-col-md3 layui-col-xs12">
				...
				<div class="layui-input-block newsStatus">
					<select name="role_id">
						@forelse($roles as $role)
							<option value="{{$role['id']}}" @if($detail['role_id'] == $role['id']) selected @endif>{{$role['name']}}</option>
						@empty
						@endforelse
					</select>
				</div>
		...
		<div class="border">
			<div class="layui-form-item newsTop">
				...
				<div class="layui-input-block">
					<input type="checkbox"  @if($detail['role_id'] == $role['id']) checked @endif name="active" value="1" lay-skin="switch" lay-text="激活|禁用">
				</div>
			</div>
...
@section('script')
    <script type="text/javascript">
        ...
        //存储路由
		var addUrl = "{{route('users.update',['id'=>$id])}}",
			hasimg = true, //检测是否传图片
